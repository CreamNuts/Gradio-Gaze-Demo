...
class LandmarkEstimator:
    def __init__(self, config: DictConfig):
        ...
        if self.mode == "dlib":
            ...
            if not (dlib_model_path := Path(config.face_detector.dlib.model)).exists():
                dlib_model_path.parent.mkdir(exist_ok=True)
    ...
    def _detect_faces_dlib(self, image: np.ndarray) -> List[Face]:
        ...
        for bbox in bboxes:
            ...
            bbox = np.array(
                [[bbox.left(), bbox.top()], [bbox.right(), bbox.bottom()]], dtype=np.float
            )
    ...
    def _detect_faces_mediapipe(self, image: np.ndarray) -> List[Face]:
        ...
        if predictions.multi_face_landmarks:
            for prediction in predictions.multi_face_landmarks:
                pts = np.array(
                    [(pt.x * w, pt.y * h) for pt in prediction.landmark], dtype=np.float64
                )
